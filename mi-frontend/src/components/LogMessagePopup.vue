    ...
    <v-layout row justify-center>
        <v-dialog v-model="dialog" max-width="600">
            <v-card class="organisationPopup" style="height: calc(100vh - 160px);">
                <v-card-title class="headline">Logging Message</v-card-title>
                <v-card-text>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      send: function () {
        console.log('Sending')
        fetchService.performPostJson('sendLogMsg?level=' + this.level, this.message).then(() => {
          this.cancel();
    ...
    created () {
      const vm = this
      eventBus.$on(EventName.LOG_POPUP, function (value) {
